    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if len(link_processed.entries) >= 1:
                ...
                entry_message = update.effective_message, "\n\n<b>Entry Title::</b> \n{}" \
            ...
            update.effective_message.reply_text(update.effective_message, "This link is not an RSS Feed link")
    else:
        update.effective_message.reply_text(update.effective_message, "URL missing")
...
def list_urls(bot, update):
    ...
    if len(final_content) == 0:
        bot.send_message(chat_id=tg_chat_id, text=(update.effective_message, "This chat is not subscribed to any links"))
    elif len(final_content) <= constants.MAX_MESSAGE_LENGTH:
        bot.send_message(chat_id=tg_chat_id, text=(update.effective_message, "This chat is subscribed to the following links:\n") + final_content)
    else:
        bot.send_message(chat_id=tg_chat_id, parse_mode=ParseMode.HTML,
                         text=(update.effective_message, "<b>Warning:</b> The message is too long to be sent"))
...
def add_url(bot, update, args):
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if row:
                update.effective_message.reply_text(update.effective_message, "This URL has already been added")
            else:
                sql.add_url(tg_chat_id, tg_feed_link, tg_old_entry_link)

                update.effective_message.reply_text(update.effective_message, "Added URL to subscription")
        else:
            update.effective_message.reply_text(update.effective_message, "This link is not an RSS Feed link")
    else:
        update.effective_message.reply_text(update.effective_message, "URL missing")
    ...
    if len(args) >= 1:
        ...
        if link_processed.bozo == 0:
            ...
            if user_data:
                ...
                update.effective_message.reply_text(update.effective_message, "Removed URL from subscription")
            else:
                update.effective_message.reply_text(update.effective_message, "You haven't subscribed to this URL yet")
        else:
            update.effective_message.reply_text(update.effective_message, "This link is not an RSS Feed link")
    else:
        update.effective_message.reply_text(update.effective_message, "URL missing")
        ...
        if len(new_entry_links) < 5:
            ...
            for link, title in zip(reversed(new_entry_links), reversed(new_entry_titles)):
                ...
                if len(final_message) <= constants.MAX_MESSAGE_LENGTH:
                    ...
                    try:
                        bot.send_message(chat_id=tg_chat_id, text=(tg_chat_id, "<b>Warning:</b> The message is too long to be sent"),
            ...
            try:
                bot.send_message(chat_id=tg_chat_id, parse_mode=ParseMode.HTML,
                             text=(tg_chat_id, "<b>Warning: </b>{} occurrences have been left out to prevent spam")
