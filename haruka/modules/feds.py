    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text(
                "Federation creation failed! This rarely happens! Ask in @HarukaAyaGroup for help!")
            return
...
def user_join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if getuser:
            update.effective_message.reply_text(
                "I cannot promote users who are already federation admins! But I can demote them if you want!")
            return
        if user_id == bot.id:
...
def user_demote_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if is_user_fed_owner(fed_id, user.id):
        ...
        if user_id == bot.id:
            update.effective_message.reply_text(
                "Are you trying to demote me as a federation admin using myself? Do you think I am stupid?")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text(
                "Big F! There is an error while setting federation rules! If you wondered why please ask it in @onepunchsupport !")
            return
...
def fed_ban_list(bot: Bot, update: Update, args: List[str], chat_data):
    ...
    getfban = sql.get_all_fban_users(fed_id)
    if len(getfban) == 0:
        update.effective_message.reply_text("The federation ban list of {} is empty".format(info['fname']),
                                            parse_mode=ParseMode.HTML)
        return
    ...
    if args:
        if args[0] == 'json':
                ...
                if jam <= int(cek.get('value')):
                    ...
                    update.effective_message.reply_text(
                        "You can backup your data once every 30 minutes!\nYou can back up data again at `{}`".format(
                            waktu), parse_mode=ParseMode.MARKDOWN)
                    return
                else:
            ...
            for users in getfban:
                getuserinfo = sql.get_all_fban_users_target(fed_id, users)
                backups += "{user_id},{first_name},{last_name},{user_name},{reason}".format(user_id=users,
                                                                                            first_name=getuserinfo[
                                                                                                'first_name'],
                                                                                            last_name=getuserinfo[
                                                                                                'last_name'],
...
def fed_chats(bot: Bot, update: Update, args: List[str]):
    ...
    if len(getlist) == 0:
        update.effective_message.reply_text("No users are fbanned from the federation {}".format(info['fname']),
                                            parse_mode=ParseMode.HTML)
        return
