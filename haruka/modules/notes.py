    ...
    if not sql.get_note(chat_id, note_name):
        ...
        msg.reply_text(
            "Ok, added `{note_name}` note in *{chat_name}*.\nGet it with `/get {note_name}`, or `#{note_name}`".format(
                note_name=note_name, chat_name=chat_name), parse_mode=ParseMode.MARKDOWN)
    else:
        sql.add_note_to_db(chat_id, note_name, text, data_type, buttons=buttons, file=content)
        msg.reply_text(
...
def clear(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if sql.rm_note(chat_id, notename):
            update.effective_message.reply_text("Note succesfully removed from *{}*.".format(chat_name),
