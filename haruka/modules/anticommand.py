...
def rem_cmds(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not args:
        ...
        if del_pref:
            update.effective_message.reply_text("Currently, The Commandzzz are being deleted.")
        else:
            update.effective_message.reply_text("Currently, The Commandzzzz Aren't being deleted.")
        return ""
    ...
    if args[0].lower() in ("on", "yes"):
        ...
        update.effective_message.reply_text("I didn't Understand what are you trying to say... 'on/yes' Or 'off/no' Include it and try again!")
        return ""
